...
?>
    ...
    <tr id="<?php echo $row['enrollment_id']?>">
        ...
        <td style="width:100px;"><a href="viewstudent_enrolledsubject.php?student_number=<?php echo $row['student_number']?>&&schoolyear=<?php echo $row['schoolyear']?>">ADD/DROP</a></td>
        <?php
        ...
        ?>
            <td style="width:100px;"><button type="submit" class="approved"name="approved"value="APPROVED"id="<?php echo $row['enrollment_id'];?>" onclick = "confirm('are you sure?') && approved(<?php echo $row['enrollment_id']; ?>);"><?Php echo $row['enrollment_status']?></button></td>
        <?php
            }
            
            else if(mysqli_num_rows($sql_ifApproved)>0){
          ?>
            <td style="width:100px;"><button disabled type="submit" class="approved"name="approved"value="APPROVED"id="<?php echo $row['enrollment_id'];?>" onclick = "confirm('are you sure?') && approved(<?php echo $row['enrollment_id']; ?>);"><i class="fa fa-ban" aria-hidden="true"style="color:red"></i><?Php echo $row['enrollment_status']?></button></td>
          <?php      
            }
